...
The audit command must complete and return machine-readable JSON. Any reported vulnerability fails the gate unless it is matched exactly by a reviewed entry in [`docs/dependency-vulnerability-exceptions.json`](docs/dependency-vulnerability-exceptions.json). The repository's exception requirements are defined in the [dependency vulnerability exception policy](docs/DEPENDENCY_VULNERABILITY_EXCEPTIONS.md). Each exception must document the package ID, advisory URL, scope, reason, and mitigation; incomplete or broad exceptions are rejected by the audit script. Exceptions are temporary risk records, not a substitute for remediation.
